...
function getRecipients(message: ForwardableEmailMessage, parsed: Awaited<ReturnType<typeof PostalMime.parse>>): string[] {
  ...
  return Array.from(new Set([...parsedTo, headerTo].filter(Boolean)));
}
...
function createRowId(messageId: string | null, toEmail: string): string {
  if (!messageId) return crypto.randomUUID();

  const source = `${messageId}:${toEmail}`;
  let hash = 0;
...
async function storeEmail(env: Env, message: ForwardableEmailMessage): Promise<void> {
  ...
  for (const toEmail of recipients) {
    ...
    await env.MAIL_DB.prepare(
      ...
      .bind(createRowId(messageId, toEmail), messageId, fromEmail, toEmail, subject, textBody, htmlBody, receivedAt)
      .run();
  }
}
